...
public class GameObject
{
	...
	// used for ForwardAction and BackAction
	...
	*/
	...
	public void LateralAction(float moveSpeed){
        oldPosition = this.getWorldLocation();
	...
	public void BackAction(float moveSpeed){
        ...
        fwdDirection.mul(this.getWorldRotation());
        fwdDirection.mul(moveSpeed);
        newPosition = oldPosition.sub(fwdDirection.x(),fwdDirection.y(),fwdDirection.z());
        this.setLocalLocation(newPosition);
	}

	/** Turn object left or right */
	public void YawAction(float rotationAmount){
		oldRotation = new Matrix4f(this.getWorldRotation());
        oldUp = new Vector4f(0f,1f,0f,0f).mul(oldRotation);
        rotAroundAvatarUp = new Matrix4f().rotation(rotationAmount, new Vector3f(oldUp.x(),oldUp.y(),oldUp.z()));
	...
	public void globalYawAction(float rotationAmount){
		oldRotation = new Matrix4f(this.getWorldRotation());
		rotAroundAvatarUp = new Matrix4f().rotation(rotationAmount, new Vector3f(0,1,0));
		newRotation = rotAroundAvatarUp.mul(oldRotation);
		this.setLocalRotation(newRotation);
	}
	

	/** Look up or down with object */
	public void PitchAction(float pitchAmount){
		oldPitch= new Matrix4f(this.getWorldRotation());
        oldPitchVec = new Vector4f(1f,0f,0f,0f);
        pitchAroundAvatarUp = new Matrix4f().rotation(pitchAmount, new Vector3f(oldPitchVec.x(),oldPitchVec.y(),oldPitchVec.z()));
        newPitch = oldPitch;
        newPitch.mul(pitchAroundAvatarUp);
        this.setLocalRotation(newPitch);
	}

	/** Roll object left or right */
	public void RollAction(float rotationAmount){
		oldRoll = new Matrix4f(this.getWorldRotation());
        oldRollVec = new Vector4f(0f,0f,-1f,0f);
        rollAroundAvatar = new Matrix4f().rotation(rotationAmount, new Vector3f(oldRollVec.x(),oldRollVec.y(),oldRollVec.z()));
        newRoll = oldRoll;
        newRoll.mul(rollAroundAvatar);
        this.setLocalRotation(newRoll);
	}
