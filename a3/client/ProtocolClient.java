...
public class ProtocolClient extends GameConnectionClient
{
	...
	{	String strMessage = (String)message;
		...
		if(messageTokens.length > 0)
		{
			...
			if (messageTokens[0].compareTo("yaw") == 0) {
                UUID ghostID = UUID.fromString(messageTokens[1]);
                float rotation = Float.parseFloat(messageTokens[2]);
	...
	{	try 
		{	String message = new String("move," + id.toString());
			...
			message += "," + position.y();
			message += "," + position.z();
			System.out.println("Sending move message: " + message); // Debug
			sendPacket(message);
		} catch (IOException e) 
		{	e.printStackTrace();
	}	}
	...
	public void sendYawMessage(float rotation) {
        try {
            ...
            message += "," + rotation;
			System.out.println("Sending rotation message: " + message); // Debug
            sendPacket(message);
        } catch (IOException e) {
            e.printStackTrace();
        }
